...
def add_new_record_to_house_db(conn, sql_cursor, result, table):
    sql_cursor.execute('''INSERT INTO ''' + table + ''' (name, longitude, latitude, province_name, city_name, district_name, house_type, area, total_price, source, create_time,
    count) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ''',result)
...
def add_new_record_to_community_db(conn, sql_cursor, result, table):
    sql_cursor.execute('''INSERT INTO ''' + table + ''' (name, longitude, latitude, province_name, city_name, district_name, province, city, district, average, metre_average, area_average, create_time,source
    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ''',result)
...
def update_house_price_db(db,cursor,results,table):
    for result in results:
        ...
        sql = '''SELECT * FROM ''' + table + ''' WHERE name={0} and area={1} and city_name={2} and house_type={3} '''.format(name,area,city,house_type)
        cursor.execute(sql)
        tmp = cursor.fetchall()
        print(len(tmp))
        if len(tmp) == 0:
            ...
            count = '"' + str(result[11]) + '"'
            update_sql = '''UPDATE ''' + table + ''' SET total_price={0}, update_time={1}, count={2} WHERE name={3} and area={4} and city_name={5} and house_type={6} '''.format(total_price,update_time,count,name, area, city, house_type)
...
def update_community_db(db,cursor, results,table):
    for result in results:
        ...
        sql = '''SELECT * FROM ''' + table + ''' WHERE name={0} and area_average={1} and city_name={2} '''.format(name,area_average,city)
        cursor.execute(sql)
        tmp = cursor.fetchall()
        print(len(tmp))
        if len(tmp) == 0:
            ...
            update_sql = '''UPDATE ''' + table + ''' SET average={0}, update_time={1}, metre_average={2} WHERE name={3} and area_average={4} and city_name={5} '''.format(
                average, update_time, metre_average, name, area_average, city)
            cursor.execute(update_sql)
    # Make commitment to the above query
    db.commit()
