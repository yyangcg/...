...
def get_city_house(user_in_nub,city):
    ...
    for i in generate_allurl(user_in_nub,city):
        ...
        for content in contents:
            ...
            district_code, district = get_district_code(city_name,district,city_code_dict)
            for ele in detail:
                try:
                    house_type = ele[0]
                    area = str(ele[1]).replace('m', '')
                    total_price = int(ele[2])
                    count = ele[3]
                    result = [name, longitude, latitude, province, city_name, district, house_type, area, total_price, source, create_time, update_time, count]
                    results.append(result)
                except:
                    pass
            # writer_to_text(results)    #储存到text文件
            # print(results)
            update_house_price_db(db,cursor,results,table='''house_price''')
            try:
                average, metre_average, area_average = get_price(results)
                result_sum.append([name, longitude, latitude, province, city_name, district, province_code, city_code, district_code,average, metre_average, area_average, create_time,update_time,source])
            except:
                pass
        # writer_to_text(result_sum)    #储存到text文件
        try:
            print('community:%s' % result_sum)
        except:
            pass

        update_community_db(db, cursor, result_sum, table='''community''')
    # 关闭数据库连接
    db.close()
