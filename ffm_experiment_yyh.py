...
from dsp.ctr.evaluation import evaluation_for_xlearn,evaluation_for_xlearn_yyh
...
class FFMExperiment(object):
        ...
        if phase == "complete":
            ...
            if phase == "train_python":
              # self.train_ffm(workdir + "ffm/ffm_train_python.txt", workdir + "ffm/ffm_test_python.txt",
               #               workdir + "ffm/")
                for i in range(28):
                    evaluation_for_xlearn_yyh(workdir + "ffm/ffm_test_python_yyh_{}.txt".format(i),
                                           workdir + "ffm/test_predict_yyh_{}.txt".format(i))
            elif phase == "ensemble":
    ...
    def train_ffm(self, trainset_path, testset_path, model_save_path, suffix=""):
        ...
        with open('/data/yangyuhan/workspace/ctr/hdfs_ctr/ffm/ffm_test_python.txt', "r", encoding="utf-8") as file_read:
            data = [line.split(' ') for line in file_read]
        field_all = [str(i) + ":" for i in range(28)]
        for i in range(len(field_all)):
            field=field_all[i]
            test_yyh_path = self.config_params["workdir"][platform.system()] +'ffm/ffm_test_python_yyh_{}.txt'.format(i)
    ...
    def eval_yyh(self,model_binary_path_list,n,testset_path):
        ...
        ffm_model.predict(model_binary_path_list[0], workdir + "ffm/test_predict_yyh_{}.txt".format(n))
    ...
    def train_ensemble(self, dataframe):
        ...
        self.evaluation([workdir + "ffm/ffm_{}.out".format(i) for i in range(1, model_cnt + 1)],
